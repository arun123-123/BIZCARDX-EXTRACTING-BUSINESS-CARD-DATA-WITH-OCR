...
if select == "Home":
  ...
  st.write(
              "### The main purpose of Bizcard is to automate the process of extracting key details from business card images, such as the name, designation, company, contact information, and other relevant data. By leveraging the power of OCR (Optical Character Recoginition) provided by EasyOCR, Bizcard is able to extract from the images.")
  ...
  if method == "Preview":
    ...
    select_query = "SELECT * FROM business_card_details"
    ...
    if button_3:
      ...
      cursor.execute(f"DELETE FROM business_card_details WHERE NAME = '{selected_name}'")
      mydb.commit()
      ...
      st.success("MODIFYED SUCCESFULLY")
  ...
  with col2:
    ...
    select_query = f"SELECT DESIGNATION FROM business_card_details WHERE NAME = '{name_select}'"
    ...
    designation_select = st.selectbox("Select the desgination ", designations) 
  ...
  if name_select and designation_select:
    ...
    with col2:
      ...
      if remove:
        cursor.execute(f"DELETE FROM business_card_details WHERE NAME = '{name_select}' AND DESIGNATION = '{designation_select}'")  
        mydb.commit()
